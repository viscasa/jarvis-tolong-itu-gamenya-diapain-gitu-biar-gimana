...
@export_range(0.1, 1.0) var spread_angle: float = 1 
...
func shoot_projectiles() -> void:
	...
	for i in range(bullet_multiplier):
		...
		_fire_one_projectile(initial_dir.rotated(offset_angle), stats) 
		_fire_one_projectile(initial_dir.rotated(-spread_angle + offset_angle), stats) 
		_fire_one_projectile(initial_dir.rotated(spread_angle + offset_angle), stats) 
